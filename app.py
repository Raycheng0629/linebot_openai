...
def weather(address):
    ...
    for item in location:
        try:
            ...
            if "降雨機率" in note:
                rain_info = note.split('降雨機率')[1].split('%')[0]
                rainfall_chance = int(rain_info)
        except Exception as e:
            print(e)
    ...
    try:
        ...
        if type == 'text':
            ...
            if text == '雷達回波圖' or text == '雷達回波':
                ...
                weather_info = user_weather.get(user_id)
                if weather_info and weather_info[1] > 40:
                    line_bot_api.reply_message(reply_token, TextSendMessage(text="請攜帶雨具"))
                else:
                    line_bot_api.reply_message(reply_token, TextSendMessage(text="多穿衣服"))
        elif type == 'location':
            address = json_data['events'][0]['message']['address'].replace('台', '臺')
            reply, rainfall_chance = weather(address)
            text_message = TextSendMessage(text=reply)
            line_bot_api.reply_message(reply_token, text_message)
            user_weather[user_id] = (address, rainfall_chance)
...
def handle_location(event):
    ...
    weather_forecast, rainfall_chance = weather(address)
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text=weather_forecast)
    )
    user_weather[event.source.user_id] = (address, rainfall_chance)
...
def handle_message(event):
    ...
    if "天氣預報" in message:
        ...
        weather_info = user_weather.get(event.source.user_id)
        if weather_info and weather_info[1] > 40:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="請攜帶雨具"))
