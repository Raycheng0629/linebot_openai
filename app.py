    ...
    try:
        for i in result:
            if i in address:
                ...
                if "降雨機率" in result[i]:
                    rain_info = float(result[i].split("降雨機率")[1].split(" ")[0])
                break
    except Exception as e:
        print(e)
    ...
    try:
        ...
        if type == 'text':
            ...
            if text == '雷達回波圖' or text == '雷達回波':
                ...
                news_message = "\n\n".join([f"{i+1}. {news['title']}\n{news['url']}" for i, news in enumerate(news_data)])
            ...
            reply, rain_info = weather(address)
            text_message = TextSendMessage(text=reply)
            line_bot_api.reply_message(reply_token, text_message)
            if rain_info > 40:
                line_bot_api.push_message(user_id, TextSendMessage(text="請攜帶雨具"))
