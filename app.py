...
def generate_health_advice(weather_info, temp, rain_prob, aqi):
    ...
    if temp > 30:
        ...
        advice.append(f"溫度{temp}度 :\n穿著：舒適、透氣的衣物，如T恤、薄長褲或裙子，攜帶輕便外套以應對早晚溫差。\n活動：適合各類戶外活動，注意適時補充水分。")
    elif 10 <= temp < 20:
        advice.append(f"溫度{temp}度 :\n穿著：長袖衣物，薄外套或風衣。\n活動：適合戶外運動，但早晚可能需要加穿外套。")
    elif temp < 10:
    ...
    # AQI advice
    if 0 <= aqi <= 50:
        advice.append("AQI值{}:\n空氣質量優良，適合所有戶外活動。".format(aqi))
    elif 51 <= aqi <= 100:
        advice.append("AQI值{}:\n空氣質量尚可，一般人群可以正常活動，敏感人群（如老人、小孩、呼吸道疾病患者）外出時應減少高強度戶外活動。".format(aqi))
    elif 101 <= aqi <= 150:
        advice.append("AQI值{}:\n敏感人群應減少戶外活動，戴口罩，一般人群注意適當減少高強度戶外活動。".format(aqi))
    elif 151 <= aqi <= 200:
        advice.append("AQI值{}:\n所有人群應減少戶外活動，建議戴口罩，尤其是在戶外運動時。".format(aqi))
    elif 201 <= aqi <= 300:
        advice.append("AQI值{}:\n避免外出，尤其是老年人、兒童和有呼吸道疾病的人，必須外出時戴N95口罩。".format(aqi))
    elif aqi > 300:
        advice.append("AQI值{}:\n避免所有戶外活動，留在室內，關閉門窗，使用空氣淨化器，準備應急物品。".format(aqi))
